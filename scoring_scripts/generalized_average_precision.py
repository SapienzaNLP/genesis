...
class GeneralizedAveragePrecision(object):
    ...
    def calc(gold_vector, evaluated_vector, random=False):  
        ...
        for [key, value] in gold_vector:
            gold_map[key]=value
        sorted_gold_vector = sorted([(k,v) for k,v in gold_vector], key=lambda x:x[1], reverse=True)
        ...
        if random is False:
            sorted_evaluated_vector = sorted([(k,v) for k,v in evaluated_vector], key=lambda x:x[1], reverse=True)
        else:
        ...
        for (key, accum_score) in evaluated_vector_accumulated:
            i += 1
            if key in gold_map and gold_map[key] > 0:
    ...
    def calcTopN(gold_vector, evaluated_vector, n, measure_type):  
        ...
        gold_top_score_sum = sum([float(score) for (key, score) in gold_vector_sorted[:n]])
                  
        evaluated_top_score_sum = 0
        sorted_evaluated_vector = sorted([(k,v) for k,v in evaluated_vector.items()], key=lambda x:x[1], reverse=True)
